...
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
##@Version           :  202307042120-git
# @@Author           :  Jason Hempstead
# @@Contact          :  jason@casjaysdev.com
# @@License          :  LICENSE.md
# @@ReadME           :  install.sh --help
# @@Copyright        :  Copyright: (c) 2023 Jason Hempstead, Casjays Developments
# @@Created          :  Tuesday, Jul 04, 2023 21:20 EDT
# @@File             :  install.sh
# @@Description      :  Container installer script for rarbg
...
HUB_IMAGE_URL="casjaysdevdocker/rarbg"
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
...
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
# Set container hostname and domain - Default: [rarbg.$SET_HOST_FULL_NAME] [$SET_HOST_FULL_DOMAIN]
CONTAINER_HOSTNAME=""
...
CONTAINER_ADD_CUSTOM_PORT="0.0.0.0:3333:3333"
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
...
CONTAINER_HOSTNAME="${CONTAINER_HOSTNAME:-${APPNAME:-rarbg}}"
...
if [ -n "$RARBG_USERNAME" ]; then
  CONTAINER_USER_NAME="$RARBG_USERNAME"
fi
if [ -n "$CONTAINER_USER_NAME" ]; then
  CONTAINER_USER_NAME="${RARBG_USERNAME:-${CONTAINER_USER_NAME:-$DEFAULT_USERNAME}}"
fi
if [ -n "$CONTAINER_USER_NAME" ]; then
...
if [ -n "$RARBG_PASSWORD" ]; then
  CONTAINER_USER_PASS="$RARBG_PASSWORD"
fi
if [ -n "$CONTAINER_USER_PASS" ]; then
  if [ "$CONTAINER_USER_PASS" = "random" ]; then
    CONTAINER_USER_PASS="$(__password "${CONTAINER_PASS_LENGTH:-16}")"
  fi
  CONTAINER_USER_PASS="${RARBG_PASSWORD:-${CONTAINER_USER_PASS:-$DEFAULT_PASSWORD}}"
fi
if [ -n "$CONTAINER_USER_PASS" ]; then
